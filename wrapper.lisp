...
(defun lispify (name flag &optional (package *package*))
  (labels ((helper (lst last rest &aux (c (car lst)))
    ...
    (let* ((fix (case flag
                  ...
                  (variable "*")
                  (t "")))
           (symstr (concatenate 'string fix (nreverse (helper (concatenate 'list (strip-prefix "_fuhfuh_dontstrip" name)) nil nil)) fix))
           ;; test for "modern" lisp
           (case-insensitive-p (eq (intern "MYSYM" :sodium) 'sodium::mysym)))
